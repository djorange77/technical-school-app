...
/**
 * This class resembles a database. It maintains an instance 
 * variable called "offerings" which stores a collection of 
 * Modules offered in Semester One. It contains methods such 
 * as an accessor method, a search-by-code method, 
 * a method that initialises data onto the array, 
 * and a certification method.  
 * 
...
public class TechnicalSchool 
{
	...
	/**
	 * The default constructor for TechnicalSchool class. It
	 * initialises the instance data (by calling a static  
	 * method in TechnicalSchool class) called "offerings" to 
	 * an array of modules specified in the static method. 
	 **/
	...
	//--------------------------------------------------------
	/**
	 * Returns an array of modules offered by the school 
	 * to caller. 
	 * 
	 * This method returns the instance data (an array of Modules)
	 * to caller, the information can later be used to display 
	 * the papers offered by the school.
	 * 
	 * @return offerings : A Module array consisting the current
	 * 	 					semester's modules.					
	 **/
	...
	//--------------------------------------------------------
	/**
	 * Returns either a Module object or null (depending on
	 * conditions) to caller. The returned object can be used to
	 * assign specific papers (Modules) to a Student's transcript
	 * without having to manually enter its full details.
	 * 
	 * This method allows the user to enter a code to search  
	 * for a module offered by the school. This method iterates 
	 * through "offerings" array in an attempt to match a paper 
	 * with the code specified. If the module is part of the 
	 * semester's offerings, it returns that specific module  
	 * to caller, otherwise it returns null if it is not found. 
	 * 
	...
	/**
	 * Returns an array of Modules comprising the 13 papers 
	 * (modules) that are offered by the school in semester one.
	 * 
	 * This is a static method for TechnicalSchool class. 
	 * It creates an array of Modules, comprising of 13 distinct  
	 * new instances of Modules (as specified in the Assignment 
	 * brief) that are offered by the school. The populated
	 * array is then returned to caller. 
	 * 
	 * @return semesterOneModules : an array of modules that are
	...
	/**
	 * Returns a boolean value (T/F) stating whether if the 
	 * student has met certain specific requirements or not. 
	 * This information can be used to determine if a student  
	 * has passed the course.
	 * 
	 * This method retrieves a copy of the student's transcript,
	 * counts their passed papers from each level and counts
	 * specific ModuleTypes achieved. It then evaluates the student
	 * based on the criteria specified in the Assignment's brief,
	 * returning a boolean value describing their certification status.
	 *  
	 * @param student : A Student object containing their
	 * 					name, transcript, and number of results
	 * @return a boolean value stating whether if the student
	 * 			is certified or not
	 **/
	public boolean isCertified(Student student)
	{
		...
		if((levelRequisites[Level.ONE.ordinal()] >= 3) && 			// Level 1 Requirements
			(levelRequisites[Level.TWO.ordinal()] >= 3 && typeCoRequisites[ModuleType.SELF_STUDY.ordinal()] > 1) && // Level 2 Requirements and Co-Requirements
			(levelRequisites[Level.THREE.ordinal()] >= 4 && typeCoRequisites[ModuleType.TAUGHT.ordinal()] >= 2) &&  // Level 3 Requirements and Co-Requirements
